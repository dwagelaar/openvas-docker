...
[![Docker Build](https://github.com/dwagelaar/openvas-docker/actions/workflows/docker-image.yml/badge.svg)](https://github.com/dwagelaar/openvas-docker/actions/workflows/docker-image.yml) [![Docker Pulls](https://img.shields.io/docker/pulls/dwagelaar/openvas.svg)](https://hub.docker.com/r/dwagelaar/openvas/) [![Docker Stars](https://img.shields.io/docker/stars/dwagelaar/openvas.svg)](https://hub.docker.com/r/dwagelaar/openvas/) [![](https://images.microbadger.com/badges/image/dwagelaar/openvas.svg)](https://microbadger.com/images/dwagelaar/openvas "Get your own image badge on microbadger.com")

A Docker container for OpenVAS on Ubuntu, based on [mikesplain/openvas-docker](https://github.com/mikesplain/openvas-docker).  By default, the latest images includes the OpenVAS Base as well as the NVTs and Certs required to run OpenVAS.  We made the decision to use 9 as the default branch.  Our Openvas9 build was designed to be a smaller image with fewer extras built in.
...
```
# latest (9)
docker run -d -p 443:443 --name openvas dwagelaar/openvas
# 9
docker run -d -p 443:443 --name openvas dwagelaar/openvas:9
```
...
```
docker run -d -p 443:443 -e PUBLIC_HOSTNAME=myopenvas.example.org --name openvas dwagelaar/openvas
```

#### OpenVAS Manager
To use OpenVAS Manager, add port `9390` to you docker run command:
```
docker run -d -p 443:443 -p 9390:9390 --name openvas dwagelaar/openvas
```
...
```
mkdir data
docker run -d -p 443:443 -v $(pwd)/data:/var/lib/openvas/mgr/ --name openvas dwagelaar/openvas
```
...
```
docker run -d -p 443:443 -e OV_PASSWORD=securepassword41 --name openvas dwagelaar/openvas
```
#### Update NVTs
...
I'm always happy to accept [pull requests](https://github.com/dwagelaar/openvas-docker/pulls) or [issues](https://github.com/dwagelaar/openvas-docker/issues).

Thanks
------
Thanks to:
* mikesplain for the great base image: https://github.com/mikesplain/openvas-docker
* hackertarget for the great tutorial: http://hackertarget.com/install-openvas-7-ubuntu/
* Serge Katzmann for contributing with some great work on OpenVAS 8: https://github.com/sergekatzmann/openvas8-complete
